...
function VMCard({ vm, index, isSelected, onSelect, onDelete, onView }) {
  return (
    <motion.div
      ...
      className="bg-card/40 border border-border rounded-xl p-4 hover:border-border/80 transition-colors"
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-8 w-8 text-foreground/60 hover:text-primary hover:bg-primary/10"
          ...
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-8 w-8 text-foreground/60 hover:text-red-400 hover:bg-red-500/10"
            onClick={() => onDelete(vm)}
          >
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div className="flex items-center gap-2 text-foreground/60 col-span-2">
          ...
          <CardContent className="p-0">
            <AnimatePresence>
              {selectedVMs.length > 0 && (
                <motion.div 
                  ...
                  className="flex items-center gap-4 px-4 md:px-6 py-3 bg-primary/10 border-b border-border overflow-hidden"
                >
                  <div className="flex items-center gap-2">
                  ...
                  <Separator orientation="vertical" className="h-4 bg-foreground/20 hidden sm:block" />
                  <Button 
                    variant="ghost" 
                    size="sm"
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="border-b border-border bg-card/80 backdrop-blur-md">
                      ...
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Name</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Status</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">CPU</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Memory</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Image</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Internal IP</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">External IP</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium">Uptime</th>
                      <th className="text-left p-4 text-xs uppercase tracking-wider text-foreground/40 font-medium w-24">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {vms.map((vm, index) => (
                      <motion.tr 
                        ...
                        className="border-b border-border/50 transition-all duration-200 group relative"
                      >
                        <td 
                          className="absolute left-0 top-0 bottom-0 w-0.5 bg-primary transition-all duration-200"
                        ...
                        <td className="p-4 text-foreground/70">{vm.cpu} cores</td>
                        <td className="p-4 text-foreground/70">{vm.memory} Gi</td>
                        <td className="p-4 text-foreground/70 font-mono text-xs">{vm.image}</td>
                        <td className="p-4">
                          {vm.internalIP ? (
                            <button
                              onClick={() => handleCopyIP(vm.internalIP)}
                              className="text-foreground/70 font-mono text-xs hover:text-primary transition-colors flex items-center gap-1"
                            >
                              {vm.internalIP}
                        ...
                        <td className="p-4 text-foreground/70">{vm.uptime}</td>
                        <td className="p-4">
            ...
            <div className="md:hidden p-4 space-y-3">
              {loading ? (
                <>
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="h-40 bg-foreground/5 rounded-xl animate-pulse" />
                  ))}
                </>
              ) : (
                vms.map((vm, index) => (
            ...
            {!loading && vms.length === 0 && (
              ...
              >
                <div className="w-20 h-20 mx-auto mb-6 flex items-center justify-center bg-card/50 rounded-2xl border border-border">
                  <Server className="w-8 h-8 text-foreground/30" />
                </div>
                <h3 className="text-foreground font-medium mb-2">No VMs Found</h3>
        ...
        <DialogContent className="bg-card border-border max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-foreground/60">
              Are you sure you want to delete <span className="text-foreground font-medium">{vmToDelete?.name}</span>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button 
              ...
              className="text-foreground/60 w-full sm:w-auto"
            >
              Cancel
            </Button>
            <Button 
